...
![推箱子](/images/技术博客/益智游戏开发-推箱子/推箱子.png)
...
- 游戏界面如何绘制？
  ...
  游戏界面在一个`m * n`（默认是 12 * 9）的区域内进行，初始化一个二维数组，将区域内的每个格子的`i、j`坐标与二维数组的两层下标对应，数组元素对应的值对应初始游戏界面元素的类型。例如：`0`表示空白区域，可进行移动；`1`表示墙壁，无法移动；`2`表示箱子目的地。
