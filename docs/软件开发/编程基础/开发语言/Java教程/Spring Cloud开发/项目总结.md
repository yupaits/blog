...
StatsD是一个解决方案，既可以自己用开源组件搭建，又可以选择第三方商业服务商，例如[DataDog](https://www.datadoghq.com/)。应用程序自身的数据采集则需要根据使用的服务商确定。如果使用DataDog，它会提供一个dd-java-agent.jar，在启动应用程序时，以agent的方式注入到JVM中：
```bash
