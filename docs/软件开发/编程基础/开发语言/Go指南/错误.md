...
通过实现`Error()`方法，可以很轻易的自定义 error，例如`errors`包下的`errorString`就是一个很简单的实现。
...
`wrapError`同样实现了`error`接口，也多了一个方法`Unwrap`，用于返回其内部对于原 error 的引用，层层包装下就形成了一条错误链表，顺着链表上寻找，很容易就能找到原始错误。由于该结构体并不对外暴露，所以只能使用`fmt.Errorf`函数来进行创建，例如
