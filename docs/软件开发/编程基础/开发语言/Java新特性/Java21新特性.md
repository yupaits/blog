...
[Java20 新特性](./Java20新特性)已经详细介绍过记录模式，这里就不重复了。
...
外部函数和内存 API 在 Java 17 中进行了第一轮孵化，由 [JEP 412](https://openjdk.java.net/jeps/412) 提出。Java 18 中进行了第二次孵化，由[JEP 419](https://openjdk.java.net/jeps/419) 提出。Java 19 中是第一次预览，由 [JEP 424](https://openjdk.java.net/jeps/424) 提出。JDK 20 中是第二次预览，由 [JEP 434](https://openjdk.java.net/jeps/434) 提出。JDK 21 中是第三次预览，由 [JEP 442](https://openjdk.java.net/jeps/442) 提出。
...
未命名变量的典型场景是 `try-with-resources` 语句、 `catch` 子句中的异常变量和·循环。当变量不需要使用的时候就可以使用下划线 `_` 代替，这样清晰标识未被使用的变量。
...
[Java20 新特性](./Java20新特性)已经详细介绍过虚拟线程，这里就不重复了。

## JEP 445：未命名类和实例main方法 （预览）
